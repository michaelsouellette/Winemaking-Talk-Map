...
$produce = $_POST['produce'];
	...
	// Check for a nursery name:
	if (empty($_POST['full_name'])) {
		$errors[] = "You forgot to enter the nursery's name.";
	} else {
...
if (empty($errors)) { // If everything's OK.
	...
	if ($lat == "00.000000" & $long == "00.000000") {
		...
		<p>Please check and reenter the address for the nursery you just tried to submit.<br />
		</div>';		
	} else {	//If there is a value for latitude and longitude
		// Make sure the nursery is not already in the database:
		$q = "SELECT id FROM markers WHERE (lat='$lat' and lng='$long' and type='nursery')";
		...
		// Add the nursery to the database:
		$query = "INSERT INTO markers (name, address, city, state, zip, username, country, website, phone, lat, lng, type, dateadded) VALUES ('$name', '$streetadd', '$city', '$state', '$zip', '$forumun','$country', '$website', '$phone', '$lat', '$long', 'nursery', NOW())";
		$result = mysql_query ($query) or trigger_error("Query: $q\n<br />MySQL Error: " . mysql_error($dbc));
	
		$dbid = mysql_insert_id();
		$q2 = "INSERT INTO nursery (dbid, produce) VALUES ('$dbid', '$produce')";
		$r2 = mysql_query ($q2) or trigger_error("Query: $q\n<br />MySQL Error: " . mysql_error($dbc));	
		
		header("Location: index.php");
		} else { // If nursery is already in the database.
				echo '
		<div id="statusdisplay">
		<a href="index.php"><img src="images/btnclose.png" alt="Close Error Panel" class="button" align="right" /></a>
		<p>The nursery you just tried to submit is already in the database.<br />
		</div>';
		}}
	...
	<div id="statusdisplay">
	<a href="index.php"><img src="images/btnclose.png" alt="Close Error Panel" class="button" align="right" /></a>
	<p class="error">The following error(s) occurred while adding your nursery:<br />';
